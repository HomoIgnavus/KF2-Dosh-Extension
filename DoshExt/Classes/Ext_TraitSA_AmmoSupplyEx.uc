...
Class Ext_TraitSA_AmmoSupplyEx extends Ext_TraitSA_Base;

static function ApplyEffectOn(ExtHumanPawn Player, Ext_PerkBase Perk, byte Level, optional Ext_TraitDataStore Data)
{
	local Ext_PerkSupport SupportPerk;
	SupportPerk = Ext_PerkSupport(Perk);

	if (SupportPerk != None)
	{
		SupportPerk.bHasAmmoSupplyEx = true;
	}
}
...
static function CancelEffectOn(ExtHumanPawn Player, Ext_PerkBase Perk, byte Level, optional Ext_TraitDataStore Data)
{
	...
	if (SupportPerk != None)
	{
		SupportPerk.bHasAmmoSupplyEx = false;
	}
}
